...
def catalogue(request):		
	...
	return render(request, 'minesmarket/catalogue.html', {'liste_produits': liste_produits})
@login_required
def catalogue_metro(request):		
	liste_produits = Produit.objects.order_by('categorie', 'nom').filter(metro=True)
	return render(request, 'minesmarket/catalogue_metro.html', {'liste_produits': liste_produits})

@login_required
def commande(request):
	try:
		solde = request.user.profile.solde_minesmarket
		...
		liste_achats = None	
		total = 0
	return render(request, 'minesmarket/commande.html', {'commande':commande, 'liste_achats': liste_achats, 'total' : total, 'solde_minesmarket':solde})
@login_required
def acheter(request):
	if request.POST:
		try:
...
def valider_commande(request):
	commande = Commande.objects.get(eleve__user__username=request.user.username, fermee=False)
	if float(commande.total()) > request.user.profile.solde_minesmarket:
		messages.add_message(request, messages.ERROR, "Pas assez d'argent sur votre compte de MinesMarket.")
	else:
		request.user.profile.update_solde_minesmarket(commande.total())
...
def credit_eleve(request):
    ...
    return render(request, 'minesmarket/credit_eleve.html', {'form': form,})
@login_required
