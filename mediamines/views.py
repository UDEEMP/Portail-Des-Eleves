...
def albums_recents(request):
    ...
    return render(request, 'mediamines/gallery_archive.html', {})        {
...
def albums_liste(request, page):
    ...
    return render(request, 'mediamines/gallery_list.html', {})
@login_required
def album_mosaique(request, slug):
    ...
    if request.user.profile.en_premiere_annee():
        if album.is_hidden_1A:
            album = None
    return render(request, 'mediamines/gallery_detail_mosaique.html', {'album':album})
@login_required
def album_diaporama(request, slug):
    ...
    return render(request, 'mediamines/gallery_detail_diaporama.html', {'album':album, 'liste_eleves':liste_eleves, 'first_slide':first_slide, 'autoplay':autoplay})
@login_required
...
def photo_detail(request, slug):
    ...
    if request.user.profile.en_premiere_annee():
        if photo.is_hidden_1A():
            photo = None
    liste_eleves = UserProfile.objects.order_by('-promo','last_name')
    return render(request, 'mediamines/photo_detail.html', {'photo':photo, 'liste_eleves':liste_eleves})
@login_required
