...
def catalogueFresh(request):		
	return render(request, 'freshbox/catalogueFresh.html', {})

@login_required
def commande(request):
	try:
		solde = request.user.profile.solde_freshbox
		commande = Commande.objects.get(eleve__user__username=request.user.username, fermee=False)
	except Commande.DoesNotExist:
		commande = None
	return render(request, 'freshbox/commande.html', {'commande':commande, 'solde_freshbox':solde})
@login_required
def acheter(request):
	if request.POST:
		try:
...
def valider_commande(request):
	commande = Commande.objects.get(eleve__user__username=request.user.username, fermee=False)
	if float(11.0) > request.user.profile.solde_freshbox:
		messages.add_message(request, messages.ERROR, "Pas assez d'argent sur votre compte de Freshbox.")
	else:
		request.user.profile.update_solde_freshbox(11.0)
...
def credit_eleve(request):
    ...
    return render(request, 'freshbox/credit_eleve.html', {'form': form,})
@login_required
