	...
	def render(self, context):
		...
		if sondage:
			...
			if Vote.objects.filter(sondage = sondage, eleve__user__username=self.login.resolve(context)).exists(): #L'élève a déjà voté
				context['a_vote'] = True
				if sondage.id == 1693:
					context['nombre_votes'] = Vote.objects.filter(sondage = sondage).count() +1000
					context['nombre_votes_1'] = Vote.objects.filter(sondage = sondage, choix = 1).count() +1000
					context['nombre_votes_2'] = Vote.objects.filter(sondage = sondage, choix = 2).count()
				elif sondage.id == 2508:
					context['nombre_votes'] = Vote.objects.filter(sondage = sondage).count() + 2000
					context['nombre_votes_1'] = Vote.objects.filter(sondage = sondage, choix = 1).count()
					context['nombre_votes_2'] = Vote.objects.filter(sondage = sondage, choix = 2).count()+2000
				else:
