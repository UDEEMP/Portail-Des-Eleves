    ...
    if Sondage.objects.filter(deja_paru = False, date_parution = datetime.date.today()).exists(): #Le sondage du jour a déjà été choisi
        ...
        if Vote.objects.filter(sondage = sondage, eleve__user__username=request.user.username).exists(): #L'élève a déjà voté
            ...
            if request.POST['choix']:
                ...
                request.user.profile.update_sondages()
...
def proposer(request):
    if request.POST:
        ...
        return render(request, 'sondages/proposer.html', {})
...
def valider(request):
    if request.POST:
        ...
        liste_sondages = Sondage.objects.filter(autorise = False).order_by('-id')
        return render(request, 'sondages/valider.html', {'liste_sondages':liste_sondages})
@permission_required('sondages.add_sondage')
# Liste des sondages non validés
def en_attente(request):
    liste_sondages = Sondage.objects.filter(autorise = True, deja_paru = False)
    return render(request, 'sondages/en_attente.html', {'liste_sondages':liste_sondages})
...
def detail_json(request, indice_sondage):
    ...
    sondage = Sondage.objects.filter(date_parution__isnull = False).filter(date_parution__lte = datetime.date.today(), date_parution__gte = d).order_by('-date_parution')[int(indice_sondage)]
    ...
    if is_premier and not Vote.objects.filter(sondage = sondage, eleve = request.user.profile).exists(): #a vote
...
def scores(request):
    ...
    return render(request, 'sondages/scores.html', {'liste_eleves_c':liste_eleves_c,'liste_eleves_d':liste_eleves_d, 'liste_eleves_c_semaine':liste_eleves_c_semaine, 'liste_eleves_d_semaine':liste_eleves_d_semaine, 'liste_eleves_participations':liste_eleves_participations})