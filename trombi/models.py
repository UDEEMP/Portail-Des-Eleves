...
class UserProfileManager(models.Manager):
    def promos_actuelles(self, nombre=4):
        ...
        return self.filter(promo__gte = promo_max, est_isupfere = False).exclude(promo = promo_max, est_cesurien = False)

    def promos_actuelles_isup(self, nombre=3):
        """
            Les 1A, 2A, 3A Isupferes
            On peut spécifier le nombre de promos souhaitées en argument
        """
        promo_max = UserProfile.premiere_annee_isup() - (nombre)
        return self.filter(promo__gte = promo_max, est_isupfere = True)

class UserProfile(models.Model):
    user = models.OneToOneField(User, help_text="Le compte utilisateur associé au profil", related_name='profile')
    ...
    def premiere_annee():
        ...
        return UserProfile.objects.filter(est_isupfere = False).aggregate(Max('promo'))['promo__max']

    @staticmethod
    def premiere_annee_isup():
        """Renvoie le maximum des promos des élèves inscrits sur le portail"""
        from django.db.models import Max
        return UserProfile.objects.filter(est_isupfere = True).aggregate(Max('promo'))['promo__max']
    ...
    def en_premiere_annee(self):
        ...
        return (self.annee() == 1) or ((self.annee() == 2) and self.est_ast) or self.est_isupfere
