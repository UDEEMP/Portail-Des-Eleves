...
def index(request):
    ...
    return render(request, 'petitscours/index.html', {'cours_list': cours_list})    
@login_required
...
def admin(request):
    ...
    return render(request, 'petitscours/admin.html', {'cours_list': cours_list,'archive_list': archive_list})    
@permission_required('petitscours.change_petitcours')
...
def archive(request,page):
    ...
    return render(request, 'petitscours/archive.html', {'archive_list': archive_list,'previous':previous,'next':next})
@permission_required('petitscours.change_petitcours')
...
def demander(request):
    if request.method == 'POST':
        ...
        pc.age=request.POST['age']
        pc.frequence=request.POST['frequence']
        pc.save()        
        if request.is_ajax():
            return HttpResponse("OK")
        else:
            return render(request, 'petitscours/demander.html', {})
    else:
        return render(request, 'petitscours/demander.html', {})        
def demander_accueil(request):
    if request.method == 'POST':
        pc = PetitCours()
        if request.user.is_authenticated():
            pc.poste_par = request.user
        ...
        if request.is_ajax():
            ...
            return render(request, 'petitscours/demander_accueil.html', {})
    else:
        return render(request, 'petitscours/demander_accueil.html', {})