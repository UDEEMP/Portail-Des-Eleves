...
def index(request):
	...
	return render(request, 'evenement/calendrier.html', {})
@login_required
...
def nouveau_calendrier(request):
	...
	Evenement.objects.create(association = None, createur = request.user.profile, titre = request.POST['title'], description = request.POST['body'], date_debut = debut, date_fin = fin)
...
def nouveau(request, association_pseudo):
	...
	if request.POST:
		...
		if Adhesion.existe(request.user.profile, association):
			evenement = Evenement.objects.create(association = association, createur = request.user.profile, titre = request.POST['titre'], description = request.POST['description'], lieu =request.POST['lieu'], date_debut = debut, date_fin=fin)
		...
		return render(request, 'evenement/nouveau.html', {'association' : association})
@login_required
...
@login_required
def get_ics(request, evenement_id):
	"""Supprimer un événement"""
	evenement = get_object_or_404(Evenement, pk = evenement_id)
	cal = Calendar()
	evt = Event()
	descr = evenement.description
	evt.add('summary', evenement.titre + (' - ' + descr if descr else ''))
	evt.add('dtstart', evenement.date_debut)
	evt.add('dtend', evenement.date_fin)
