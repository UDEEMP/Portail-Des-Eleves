...
class ThreadCreationForm(forms.Form):
    ...
    topic = forms.ModelChoiceField(queryset=models.Topic.objects.all(),label='Section')
    title = forms.CharField(max_length=200, label='Titre')
    body = forms.CharField(label='Contenu', widget=forms.Textarea(attrs={'rows': 10, 'cols': 40}))
...
class ThreadReplyForm(forms.Form):
    ...
    body = forms.CharField(label='Répondre', widget=forms.Textarea(attrs={'rows': 10, 'cols': 50}))
