...
def nouveau(request):
    if request.method == 'POST':
        ...
        form = UploadFileForm()
    return render(request, 'minestryofsound/nouveau.html', {'form': form})
@login_required
def playlist(request):
    liste_morceaux = Morceau.objects.all()
    return render(request, 'minestryofsound/playlist.html', {'liste_morceaux': liste_morceaux})
@login_required
...
def playlist_popup(request):
    return render(request, 'minestryofsound/popup.html', {})
