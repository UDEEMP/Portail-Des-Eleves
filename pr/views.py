...
def clips(request):
	...
	if request.user.profile.en_premiere_annee() and pr.is_hidden_1A == True:
		clip_list = []
	else :
		clip_list = Clip.objects.all()
		
	return render(request, 'pr/clips.html', {'clip_list': clip_list})
...
def voter(request):
    ...
    if listes.count() >= 1: #On peut voter
        ...
        if Vote.objects.filter(eleve = request.user.profile).count()>1: #L'élève a déjà voté
            # messages.add_message(request, messages.ERROR, "Vous avez déjà voté !")
            peut_voter = False
            deja_vote = True
        elif Vote.objects.filter(eleve = request.user.profile).count()==1:
    ...
    if listes.count()>0 :
        if request.POST :
            ...
            if Candidat.objects.filter(nom = name).exists() :
                if Vote.objects.filter(eleve = request.user.profile).count()==1:
                    liste = get_object_or_404(Candidat, nom = name)
                    if Vote.objects.filter(eleve = request.user.profile,liste__nom = liste).exists() :
                ...
                if not deja_vote and Vote.objects.filter(eleve = request.user.profile).count()!=1:
                    liste = get_object_or_404(Candidat, nom = name) 
                    liste.nbVotes = liste.nbVotes+1
                    liste.save()
                    Vote.objects.create(liste = liste, eleve=request.user.profile)
...
def resultats(request):
    ...
    return render(request, 'pr/resultats.html', {'listes':listes})
@login_required
@permission_required('pr.add_liste')
